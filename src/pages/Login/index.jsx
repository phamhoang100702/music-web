...
import {getUserById, getUserInformation} from "../../services/api/user";
...
const Login = () => {
    ...
    const onFinish = (values) => {
        (async () => {
            ...
            if (infomation.status === "BAD_REQUEST") {
    ...
    useEffect(() => {
        if (getLocalStorage('user-token') != "") {
            (async () => {
                const dataDecode = await getUserInformation(getLocalStorage('user-token'));
