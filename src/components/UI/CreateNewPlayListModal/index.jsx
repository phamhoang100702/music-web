...
const CreateNewPlaylistModal = (props) => {
    ...
    let oldSearch = "";
    const debounceSearch = debounce((value, page) => {
        (async () => {
            if (value === "") {
                const response = (await searchSongByKeyword(value, page)).content.filter(
                    (i) =>
                        listChoosed.findIndex((k) => k.id === i.id) === -1
                );
                setListSearch((pre) => [...pre, ...response]);
            } else {
                const response = (await searchSongByKeyword(value, page)).content.filter(
                    (i) =>
                        listChoosed.findIndex((k) => k.id === i.id) === -1
    ...
    const onFinish = (value) => {
        ...
        (async () => {
            ...
            if(newPlaylist.content){
                dispatch(addOnePlaylistToListPlaylist(newPlaylist.content));
                form.resetFields();
                setListChoosed([]);
                setListSearch([]);
            }else {
    ...
    useEffect(() => {
        console.log(" vao day search", search);
        if (oldSearch !== search) {
            setPage(0);
        }
        oldSearch = search;
