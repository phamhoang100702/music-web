...
import {searchSingersByKeyword} from "../../../services/api/singer/index.js";
...
const SearchInputSelect = () => {
    ...
    const handleRenderOptions = (options) => {
        if (options.data.searchType === 'song') {
            return (<Row align={"middle"} onClick={() => dispatch(playOneSongNow(options.data))}>
                <Col span={4} style={{display: "flex", alignItems: "center"}}>
                    {options.data.thumbnail ? (<img
                        src={options.data.thumbnail}
                ...
                <Col span={17}>
                    <h4 style={{
                        margin: 0, padding: 0, width: "80%", overflow: "hidden", textOverflow: "ellipsis"
                    }}>{options.data.title}</h4>
                    <span style={{
                        fontSize: 12, color: "#6d6d6d"
            ...
            return (<Row align={"middle"} onClick={() => navigate(`/singer-profile/${options.data.id}`)}>
                <Col span={4} style={{display: "flex", alignItems: "center"}}>
                    {options.data.avatar ? (<img
            ...
            return (<Row align={"middle"} onClick={() => navigate(`/list-song-of-playlist/${options.data.id}`)}>
                <Col span={4} style={{display: "flex", alignItems: "center"}}>
                    <img
    ...
    return (<ConfigProvider
        theme={{
            token: {
                colorPrimary: "#31c27c", colorPrimaryHover: "#31c27c", colorPrimaryActive: "#31C27C"
            }
        }}
    >
            ...
            <Select
                ...
                onSearch={debounce((e) => {
                    (async () => {
                        setValueSearch(e);
                        if (e !== '') {
                            let dataSong = (await searchSongByKeyword(e)).content.map(i => {
                                return {
                                    ...i,
                                    searchType: 'song'
                                }
                            });
                            let dataSinger = (await searchSingersByKeyword(e)).content.map(i => {
                                return {
                                    ...i, searchType: 'singer'
                                }
                            });
                            let dataPlaylist = (await searchAllPlaylistByNameForUser(e)).content.map(i => {
                                return {
                                    ...i, searchType: 'playlist'
                                }
                            });
                            setOptions([...dataSong, ...dataSinger, ...dataPlaylist].sort((a, b) => {
                                let nameA = a.title ? a.title.toLowerCase() : (a.name) ? a.name.toLowerCase() : "";
                                let nameB = b.title ? b.title.toLowerCase() : (b.name) ? b.name.toLowerCase() : "";
